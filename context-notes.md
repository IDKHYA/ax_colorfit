...
## 2026-07-22 포스터 2차 개편

저채도 무지개 리퀴드 글라스 배경은 유지하고, 글라스 효과는 이미지 프레임과 상단 태그에만 제한했다. 세로 화면 3장은 진단 흐름, 가로 화면 4장은 분석·추천·편집·저장 흐름으로 묶었다.

하단 문구는 `진단에서 끝나지 않는 퍼스널컬러. 내 옷장으로 오늘의 코디를 완성하세요.`로 교체했다. QR은 29mm 정사각형으로 배치해 인쇄 시에도 독립적인 체험 진입점으로 보이게 했다.

## 2026-07-22 웹 포스터 프리뷰

웹 버전은 A2 파일을 수정하지 않고 별도 파일로 관리한다. 화면 폭에 따라 갤러리 열 수가 바뀌며 모바일에서는 진단 화면과 기능 화면을 한 열로 쌓는다.

## 2026-07-22 A2 포스터 V2

- 기존 포스터는 보존하고 `colorfit-a2-poster-v2.html`을 별도 생성한다.
- 모바일 캡처는 세로 비율을 유지하고 데일리룩 제작 화면도 강제 확대하거나 자르지 않는다.
- 서로 다른 화면 크기를 같은 카드 규격에 억지로 맞추지 않고 섹션별 그리드를 사용한다.
- 여백은 이미지 확대가 아니라 분석 방식, 입력 경로, 추천 근거 설명으로 채운다.

## 2026-07-22 Liquid v3 기능 그룹 재편

- 수정 기준은 `docs/poster/colorfit-poster-liquid_v3.html`이며 다른 포스터 버전은 건드리지 않는다.
- 퍼컬 측정은 `output/퍼컬 결과.png` 한 장으로 강조한다.
- 옷장 및 옷 추천은 `output/옷장.png`, `output/이미지/나만의 옷 추가.png`, `output/옷 추천.png` 세 화면을 같은 그룹에 배치한다.
- 데일리룩은 `output/이미지/나만의 데일리룩 저장 2.png`로 강조한다.
- 화면 내부가 잘리지 않도록 갤러리 이미지는 전체 표시 방식으로 통일한다.