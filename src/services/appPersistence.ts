...
import type { ClothingItem, PersonalColorRecord, SavedLookFolder, SavedOutfit, Wardrobe } from '../wardrobeTypes';
...
export interface SavedOutfitPersistence {
  ...
  loadFolders: (fallback: SavedLookFolder[]) => SavedLookFolder[];
  saveFolders: (folders: SavedLookFolder[]) => void;
  clear: () => void;
}
...
export const localAppPersistence: AppPersistence = {
  ...
  savedOutfit: {
    ...
    loadFolders: (fallback) => loadJson<SavedLookFolder[]>(STORAGE_KEYS.savedFolders, fallback),
    saveFolders: (folders) => saveJson(STORAGE_KEYS.savedFolders, folders),
    clear: () => {
      saveJson<SavedOutfit[]>(STORAGE_KEYS.saved, []);
      removeJson(STORAGE_KEYS.savedFolders);
    },
  },
};
