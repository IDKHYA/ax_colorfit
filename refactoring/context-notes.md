...
## 2026-07-16 독립 ColorFit 서비스 기준선

`colorfit_ax.zip`은 부모 저장소 없이 의존성 설치, 프로덕션 빌드, 27개 테스트 파일과 142개 테스트를 통과했다. 얼굴 촬영, 퍼컬 융합, 날씨, 추천과 브라우저 저장 로직은 보존되어 있으므로 UI 재작성이나 프론트 전면 재구축을 목표로 하지 않는다.

독립화가 실패한 범위는 ML 이미지 처리다. Git 이력에 `server/background_remove_api.py`가 없고 `server/ml_bridge.py`는 프로젝트 바깥의 부모 경로를 조회한다. Vercel 엔트리도 누끼 요청을 503으로 반환한다. 이는 GitHub 전송 중 삭제가 아니라 최초 자립 커밋 전에 ML 구현을 포함하지 않은 문제로 판단한다.

일반 사용자는 진단 결과가 없는 상태로 시작한다. 자동 분석은 초안이며 사용자 확정값이 최종 권한이다. 배포 책임은 Vercel의 공개 프론트·경량 수집 API와 별도 ML 서비스로 분리하고, ML 장애 시에도 원본 사진과 수동 입력으로 의류 등록을 완료할 수 있어야 한다.

기존 12계절 ID, 사진·설문 융합, 추천 4축, WeatherBand, localStorage·IndexedDB 저장과 ColorFit 화면 구조는 유지한다. v1과 작업 아카이브, 중복 카탈로그는 독립 인수 테스트 이후 정리한다.

## 2026-07-16 FRD 단계 1 구현 판단

이번 단계는 FRD 단계 1(독립성 복구)만 다룬다. 실제 ML 서비스 복구(FR-040/041)는 단계 2 대상이므로 구현하지 않는다.

`server/ml_bridge.py`는 `Path(__file__).resolve().parents[2]`로 `colorfit_ax` 상위 폴더의 `server/background_remove_api.py`를 파일 경로 로딩으로 찾는다. 이는 FRD 9.4 금지 항목("부모 저장소 파일을 상대 경로로 불러와서는 안 된다")에 정확히 해당하는 코드다. 실제 ML 구현이 아직 없는 상태이므로, 이 브리지를 유지한 채 부모 경로만 지우면 로컬 개발에서도 항상 라우트가 없는 상태가 되어 `api/index.py`의 명시적 503과 동작이 갈린다. 따라서 브리지 파일 자체를 제거하고, `server/app.py`가 `/api/background/remove`·`/api/clothing/extract`를 `api/index.py`와 동일한 503 안내로 직접 등록하도록 정리한다. `api/index.py`는 이 두 라우트도 `server.app.app`에서 그대로 재사용해 안내 문구가 두 곳에서 따로 관리되지 않게 한다.

신규 사용자 초기 상태 문제는 `demoUserData.ts`의 `getInitialPersonalColorState`가 저장값이 없을 때 무조건 `SOFT_SUMMER_DEMO_RESULT`를 채우는 2026-07-03 결정에서 비롯됐다. 당시에는 "촬영 없이 골든 패스를 시험"하려는 의도였지만 FRD FR-001은 이를 제품 결함으로 명시한다. `App.tsx`는 이미 `!personalColorResult`일 때 "퍼스널 컬러 측정이 필요합니다" 빈 상태를 렌더링하므로(App.tsx:497-502), 데모 시드만 제거하면 UI 쪽 추가 변경 없이 FR-001을 만족한다. 데모 데이터 자체는 삭제하지 않고 `import.meta.env.VITE_DEMO_MODE === 'true'`일 때만 쓰도록 조건화해 시연 배포 옵션은 남긴다.

ML API 기준 URL 경계(FR-042)는 `clothingImageApi.ts`의 `/api/health`, `/api/background/remove`, `/api/clothing/extract` 세 호출에만 적용한다. `/api/ingest/url`, `/api/ingest/image`는 FRD 9.1~9.2에 따라 Vercel 경량 API 책임으로 남아 이번 경계 대상이 아니다. 값이 없으면 기존과 동일하게 상대 경로를 써서 로컬 프록시·Vercel rewrite 동작을 그대로 보존한다.

ML 장애 시 수동 등록 완료(FR-034/042)는 `useManualClothing.autoAnalyzeOnUpload`가 이미 실패 시 `imageUrl`/`cutoutImageUrl`을 건드리지 않고 오류 상태만 남기는 구조라 별도 로직 변경 없이 회귀 테스트만 추가한다.

부모 `node_modules` 의존 여부는 `package.json`에 workspaces나 상위 경로 참조가 없어 코드상 문제는 없었다. 격리 검증(Git 추적 파일만 새 폴더에 clone 후 `npm ci`/`npm run build`)으로 실제로도 상위 폴더 없이 동작함을 확인해 기록으로 남긴다.

## 2026-07-16 FRD 단계 1 구현 결과

`server/ml_bridge.py`를 삭제하고 `server/app.py`에 `/api/background/remove`·`/api/clothing/extract` 503 라우트를 직접 등록했다. `api/index.py`는 자체 503 핸들러를 없애고 `SERVERLESS_ROUTE_PATHS`에 ML 경로 두 개를 추가해 `server.app.app`의 라우트 객체를 그대로 재사용한다 — 안내 문구가 한 곳(`server/app.py`)에서만 관리된다.

`demoUserData.getInitialPersonalColorState`에 `demoModeEnabled` 인자(기본값 `isDemoModeEnabled()`, `import.meta.env.VITE_DEMO_MODE === 'true'`)를 추가했다. 저장된 결과가 없고 데모 모드가 아니면 `{ result: null, history: storedHistory }`를 반환한다. 기존 저장 결과가 있는 경로는 데모 모드 여부와 무관하게 그대로 보존된다. `demoUserData.test.ts`는 FR-001 계약(신규 사용자 null, 일반 실행 기본값도 null, 데모 모드에서만 데모 결과)으로 다시 작성했다.

`clothingImageApi.ts`에 `mlApiUrl(path)` 헬퍼를 추가해 `getApiHealth`, `requestBackgroundRemoval`, `requestPrecisionExtraction` 세 호출에만 `VITE_ML_API_BASE_URL`을 적용했다. `requestUrlIngest`/`requestUrlImageBlob`는 의도적으로 그대로 두어 URL 수집이 계속 Vercel 동일 출처 경량 API를 쓰게 했다.

`useManualClothing`과 `WardrobeSection`의 저장 버튼(`disabled={!props.manual.imageUrl}`)은 이미 AI 분석 성공 여부와 무관하게 동작해 코드 변경 없이 회귀 테스트만 추가했다.

`src/vite-env.d.ts`를 새로 추가해 `import.meta.env.VITE_DEMO_MODE`/`VITE_ML_API_BASE_URL` 타입을 선언했다(Vite 표준 관례, `tsconfig.json`의 `types`에 `vite/client`가 없어 별도 선언 필요).

### 검증 결과

- 프론트: `npm run lint`(tsc --noEmit) 통과, `npm test` 29개 파일 151개 테스트 통과(기존 142 + 신규 9), `npm run build` 통과(기존과 동일한 청크 크기 경고만 유지).
- 서버: `pytest server/tests` 25개 테스트 통과(기존 21 + 신규 4, `test_vercel_entry.py` 회귀 없음).
- 격리 검증: `git stash create`로 스테이지된 트리를 커밋 오브젝트화한 뒤 `git archive`로 임시 폴더에 풀어 Git 추적 파일만으로 `npm ci`, `npm run build`, `pytest server/tests`를 모두 통과시켰다. `grep`으로 개발자 PC 절대 경로와 `parents[2]` 부모 경로 참조를 확인했고, 남은 `parents[2]`는 `server/tests/test_vercel_entry.py`(저장소 내부에서 `server/tests` → 저장소 루트로 올라가는 정상 경로)뿐이었다.
...
## 2026-07-16 배포 방향 전환 — Render 무료 대신 로컬 실행 + ngrok

보고 직후 사용자가 "ngrok으로 내 컴퓨터에서 돌리고 서버(Vercel)에는 주소만 알려주면 되지 않느냐"고 제안했다. 타당한 방향이라 판단해 장단점을 정리해 보고했다 — 장점은 RAM 제약이 사실상 사라지고 완전 무료라는 점, 단점은 컴퓨터가 꺼지거나 ngrok이 끊기면 그 시간 동안 ML이 죽는다는 점이다(다만 이미 만든 FR-042 폴백 덕에 앱 전체가 죽지는 않는다).

사용자가 이 방향으로 확정하면서 직전에 추가한 `ENABLE_CLOTH_SEGMENTATION` 저메모리 프로필(무료 512MB 호스팅 전제)이 더 이상 필요 없다고 판단해 되돌렸다. `models.py`의 `cloth_segmentation_enabled()`/조건부 `get_cloth_session()`/`models_ready()` 분기, `inference.py`의 `cloth_session is None` 폴백 분기, `GeneralOnlyProfileTest`를 제거하고 두 모델을 항상 로드하는 원래 동작으로 복귀했다. 단, onnxruntime 메모리 아레나 비활성화(`_low_memory_session_options`)는 로컬 실행에서도 메모리를 아껴 주고 부작용이 없어 그대로 유지했다 — 이 부분만은 호스팅 방식과 무관하게 유효한 개선이다.

`refactoring/ml-service-ngrok-guide.md`에 로컬 실행 → ngrok 설치/로그인 → 무료 고정 도메인 발급(재시작해도 URL 안 바뀌게) → Vercel `VITE_ML_API_BASE_URL` 환경변수 연결 → `ml_service`의 `ALLOWED_ORIGINS`를 실제 Vercel 도메인으로 좁히는 순서를 문서화했다. 아직 실제 ngrok 계정 생성·터널 실행은 하지 않았다(사용자가 나중에 직접 진행하기로 함) — 코드 변경은 없고 가이드 문서만 추가했다.

### 남은 위험 (갱신, ngrok 방향 기준)
