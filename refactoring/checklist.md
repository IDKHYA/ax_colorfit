...
### 2026-07-16 독립 ColorFit 서비스 FRD

- [x] ZIP의 Git 추적 파일과 현재 v2 핵심 기능 파일을 대조한다.
- [x] 프론트 독립 설치, 빌드, 27개 테스트 파일과 142개 테스트 통과를 기준선으로 기록한다.
- [x] 누락된 ML 구현, 잘못된 부모 경로, Vercel ML 503을 핵심 문제로 확정한다.
- [x] 기능·비기능 요구사항, 시스템 경계, 인수 기준과 위험을 포함한 FRD를 작성한다.
- [x] 사용자 승인 후 구현 단계용 세부 체크리스트를 작성한다.
- [x] 사용자 승인 전에는 애플리케이션 코드를 변경하지 않는다.

### 2026-07-16 FRD 단계 1 — 독립성 복구 구현

- [x] git status와 실제 파일 구조를 확인한다.
- [x] FRD, `docs/v2-schema-contract.md`, `docs/domain-logic-inventory.md`를 재확인한다.
- [x] `plan.md`, `checklist.md`, `context-notes.md`에 이번 단계 범위를 기록한다.
- [x] 신규 사용자 `personalColorResult`가 null이어야 하는 실패 테스트를 먼저 작성한다(FR-001).
- [x] 일반 실행에서 데모 결과가 자동 적용되지 않아야 하는 실패 테스트를 먼저 작성한다(FR-001).
- [x] 명시적 데모 모드에서만 데모 결과가 적용되는 실패 테스트를 먼저 작성한다(FR-001).
- [x] ML API 기준 URL 환경 경계 계약 실패 테스트를 먼저 작성한다(FR-042).
- [x] ML 서비스 중단에도 원본 사진 + 수동 입력으로 등록이 끝나는 회귀 테스트를 추가한다(FR-034, FR-042).
- [x] `server/ml_bridge.py`의 부모 저장소 경로 참조를 제거하는 서버 계약 테스트를 먼저 작성한다(NFR-001).
- [x] `src/vite-env.d.ts`를 추가해 `VITE_DEMO_MODE`, `VITE_ML_API_BASE_URL` 타입을 선언한다.
- [x] `demoUserData.ts`를 데모 모드 플래그 기반으로 수정한다.
- [x] `clothingImageApi.ts`가 `VITE_ML_API_BASE_URL`을 사용하게 수정한다.
- [x] `server/app.py`에 ML 미가용 503 라우트를 직접 등록하고 `ml_bridge.py`·부모 경로 의존을 제거한다.
- [x] `api/index.py`가 `server/app.py`의 ML 라우트를 재사용하도록 정리한다.
- [x] 모든 신규·기존 대상 테스트를 통과시킨다. (프론트 29개 파일 151개 테스트, 서버 25개 테스트)
- [x] `npm run lint`, `npm test`, `npm run build`를 실행한다.
- [x] `pytest server/tests/`를 실행한다.
...
### 2026-07-16 무료 호스팅 실측과 저메모리 프로필

- [x] Render/Railway/Fly.io 무료 티어 현황을 조사한다 — Render만 실질적 무료(콜드스타트 있음).
- [x] 실제 프로세스 RSS를 측정해 두 모델 로드·반복 추론 시 최대 985MB까지 증가함을 확인한다.
- [x] onnxruntime 메모리 아레나(enable_cpu_mem_arena) 비활성화를 먼저 실패 테스트로 고정한다.
- [x] rembg 세션 클래스에 저메모리 SessionOptions를 직접 주입하도록 `models.py`를 수정한다.
- [x] 아레나 비활성화 후 반복 호출에도 메모리가 평평하게 유지됨을 재측정으로 확인한다(410MB/558MB).
- [x] `ENABLE_CLOTH_SEGMENTATION` 환경변수로 저메모리(일반 누끼 전용) 배포 프로필을 추가한다.
- [x] 저메모리 프로필에서도 정밀 추출 요청이 카테고리를 지어내지 않고 정직한 일반 폴백만 쓰는지 테스트로 고정한다.
