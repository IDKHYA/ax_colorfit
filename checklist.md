...
- [x] 제공된 제품 화면 7개와 QR 이미지를 모두 연결한다.
- [x] 진단부터 데일리룩 저장까지 흐름이 읽히도록 이미지 위계를 재구성한다.
- [x] 핵심 기능 설명을 승인된 세 기능 기준으로 구체화한다.
- [x] 기존 하단 화살표 문구를 제거하고 체험 안내와 QR로 교체한다.
- [x] A2 넘침 검사와 PDF 1페이지 렌더링을 확인한다.

### 2026-07-22 웹 포스터 프리뷰

- [x] 인쇄용 포스터와 분리된 반응형 HTML을 만든다.
- [ ] 데스크톱에서 이미지 위계와 설명 가독성을 확인한다.
- [ ] 390px 모바일에서 수평 오버플로와 겹침을 확인한다.
- [x] 브라우저에서 바로 열 수 있는 최종 경로를 제공한다.


## 2026-07-22 A2 포스터 V2

- [x] 사용 이미지 8종의 원본 비율 확인.
- [x] 진단, 옷장, 코디 완성의 세 섹션 레이아웃 설계.
- [x] `나만의 옷 추가` 화면 포함.
- [x] 모바일 및 데일리룩 이미지를 자르지 않는 HTML 제작.
- [x] A2 크기와 오버플로 검증.
- [x] PDF 출력 및 렌더 이미지 육안 검수.

## 2026-07-22 Liquid v3 기능 그룹 재편

- [x] 기준 파일과 새 이미지 비율 확인.
- [x] 상단 기능 요약의 01 진단, 02 옷장, 03 코디 구조 유지.
- [x] 쇼케이스를 퍼컬 측정, 옷장 및 옷 추천, 데일리룩 세 그룹으로 교체.
- [x] 모든 쇼케이스 이미지에 `object-fit: contain` 적용.
- [x] A2 PDF 출력과 육안 검수.